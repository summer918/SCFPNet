            ...
            if self.few_shot:
                if self.seeds == 0:
                    ...
                    self.support_df = pd.read_pickle(
                        "./datasets/pascal_voc/seed{}/{}.pkl".format(self.seeds, cfg.DATASETS.TRAIN[0]))
                    print("training support_df=",
                          "./datasets/pascal_voc/seed{}/{}.pkl".format(self.seeds, cfg.DATASETS.TRAIN[0]))
            else:
                for idx, dataset_name in enumerate(cfg.DATASETS.TRAIN):
                    ...
                    if idx == 0:
                        ...
                        self.support_df = pd.concat([self.support_df, support_df_tmp], ignore_index=True)
    ...
    def generate_support(self, dataset_dict):
        ...
        for shot in range(support_shot):
            ...
            support_list = self.support_df.loc[
                (self.support_df['category_id'] == query_cls) & (~self.support_df['image_id'].isin(used_image_id)) & (
                    ~self.support_df['id'].isin(used_id_ls)), 'id']
        ...
        if support_way == 1:
            ...
            for way in range(support_way - 1):
                other_cls = self.support_df.loc[(~self.support_df['category_id'].isin(
                    used_category_id)), 'category_id'].drop_duplicates().sample().tolist()[0]
                used_category_id.append(other_cls)
                for shot in range(support_shot):
                    # Support image and box

                    support_list = self.support_df.loc[(self.support_df['category_id'] == other_cls) & (
