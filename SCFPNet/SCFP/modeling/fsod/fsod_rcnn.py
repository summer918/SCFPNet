...
class FsodRCNN(nn.Module):
    ...
    def visualize_training(self, batched_inputs, proposals):
        ...
        max_vis_prop = 20
        print(len(batched_inputs), "999999")
    ...
    def forward(self, batched_inputs):
        ...
        features_dict = {}
        # 2 2
        #         print(images.tensor.shape[0],self.support_way)
        # 进入特征提取网络
        ...
        for b_1 in range(images.tensor.shape[0]):
            ...
            for b_2 in range(self.support_way):
                ...
                # [8 8 8 8 8 8.......] 0 10
                #                 print(batched_inputs[b_1]['support_cls'],pos_begin,pos_end,images.tensor[b_1,:].unsqueeze(0).shape)
                features_dict[b_1][b_2] = self.backbone.forward_with_two_branch(images.tensor[b_1, :].unsqueeze(0),
        ...
        for i in range(B):  # batch
            ...
            # 计算query support的相似度注意力图 带有注意力的RPN
            pos_correlation = F.conv2d(query_feature_res4, pos_support_features_pool.permute(1, 0, 2, 3),
                                       groups=query_feature_res4.shape[1])  # attention map
            # torch.Size([1, 320, 38, 51]) 1 torch.Size([3, 608, 811]) torch.Size([1, 320, 1, 1]) torch.Size([1, 320, 14, 14])
            # print(pos_correlation.shape,len(query_images),query_images[0].shape,pos_support_features_pool.shape,pos_support_features.shape)
            # torch.Size([1, 320, 50, 72]) torch.Size([1, 320, 50, 72]) torch.Size([2, 320, 14, 14])
            # print(pos_correlation.shape,query_feature_res4.shape,pos_support_features.shape)
            # query_feature_res4
            ...
            pos_proposals, pos_anchors, pos_pred_objectness_logits, pos_gt_labels, pos_pred_anchor_deltas, pos_gt_boxes = self.proposal_generator(
                query_images, pos_features, query_gt_instances)  # attention rpn
            ...
            for way in range(self.support_way - 1):
                ...
                support_features = {'res4': support_features_res4}
                neg_support_features = self.roi_heads.roi_pooling(support_features,
                                                                  support_bboxes_ls[neg_begin:neg_end])
                neg_support_features_pool = neg_support_features.mean(0, True).mean(dim=[2, 3], keepdim=True)

                neg_correlation = F.conv2d(query_feature_res4, neg_support_features_pool.permute(1, 0, 2, 3),
                                           groups=query_feature_res4.shape[1])
                # neg_correlation
                neg_features = {'res4': neg_correlation}

                neg_proposals, neg_anchors, neg_pred_objectness_logits_tmp, neg_gt_labels_tmp, neg_pred_anchor_deltas_tmp, neg_gt_boxes_tmp = self.proposal_generator(
                    query_images, neg_features, query_gt_instances)
                neg_pred_class_logits_tmp, neg_pred_proposal_deltas_tmp, neg_detector_proposals_tmp = self.roi_heads(
                    query_images, query_features, neg_support_features, neg_proposals, query_gt_instances)
                ...
                if way == 0:
                    ...
                    neg_pred_proposal_deltas = torch.cat([neg_pred_proposal_deltas, neg_pred_proposal_deltas_tmp],
                                                         dim=0)
                    neg_detector_proposals += neg_detector_proposals_tmp
    ...
    def init_model_voc(self):
        if 1:
            if self.test_seeds == 0:
                support_path = './datasets/pascal_voc/voc_2007_trainval_{}_{}shot.pkl'.format(self.keepclasses,
                                                                                              self.evaluation_shot)
            elif self.test_seeds >= 0:
                support_path = './datasets/pascal_voc/seed{}/voc_2007_trainval_{}_{}shot.pkl'.format(self.test_seeds,
                                                                                                     self.keepclasses,
                                                                                                     self.evaluation_shot)
        ...
        if 1:
            if self.keepclasses == 'all':
                if self.test_seeds == 0:
                    ...
                    support_path = './datasets/coco/seed{}/full_class_{}_shot_support_df.pkl'.format(self.test_seeds,
                                                                                                     self.evaluation_shot)
            else:
                if self.test_seeds == 0:
                    support_path = './datasets/coco/{}_shot_support_df.pkl'.format(self.evaluation_shot)
                elif self.test_seeds > 0:
                    support_path = './datasets/coco/seed{}/{}_shot_support_df.pkl'.format(self.test_seeds,
                                                                                          self.evaluation_shot)
    ...
    def inference(self, batched_inputs, detected_instances=None, do_postprocess=True):
        ...
        print(len(batched_inputs),batched_inputs[0].keys(),batched_inputs[0]['file_name'],batched_inputs[0]['image_id'])
        # batched_inputs[0]["image"] = torch.tensor(cv2.imread('/media/yl/xiangmin/fct/datasets/pascal_voc/VOC2007/JPEGImages/000001.jpg')).permute(2, 0, 1)
        images = self.preprocess_image(batched_inputs)
        # batched_inputs[0]["image"]=cv2.imread('/media/yl/xiangmin/fct/2007_003778.jpg').permute(1,2,0)
        #         print(len(images))
        ...
        for cls_id, support_images in self.support_dict['image'].items():
            print(cls_id,support_images.tensor.shape)
            query_images = ImageList.from_tensors([images[0]])  # one query image
#             '/root/autodl-tmp/fct/FCT/datasets/pascal_voc/voc_2007_trainval_all1_1shot/2010_000906/0000.jpg'
            features_dict = self.backbone.forward_with_two_branch(cv2.imread('/root/autodl-tmp/fct/FCT/datasets/pascal_voc/VOC2007/JPEGImages/'+str(batched_inputs[0]['image_id'])+'.jpg'),cls_id,query_images.tensor, support_images.tensor)
            ...
            correlation = F.conv2d(query_features_res4, pos_support_features_pool.permute(1, 0, 2, 3),
                                   groups=query_features_res4.shape[1])  # attention map
        ...
        print("提取特征完毕!!!!")
        results, _ = self.roi_heads.eval_with_support(cv2.imread('/root/autodl-tmp/fct/FCT/datasets/pascal_voc/VOC2007/JPEGImages/'+str(batched_inputs[0]['image_id'])+'.jpg'), query_features_dict, support_proposals_dict,
                                                      support_box_features_dict)
